...
class InsertReturningQueryTest : TestContainersSetup() {
    ...
    fun `should insert returning`() {
        ...
        val result = repo.access { a ->
            a.execute(insertInto("insert_returning_query_test")
    ...
    fun `should support on conflict clause`() {
        ...
        // then
        val result = repo.queryOneNullable("select * from insert_returning_query_test where name = ?", listOf(text(name))) { r ->
